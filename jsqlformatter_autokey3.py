...
class SimpleLexer(object):
    ...
    def getSubstr(self, beginIndex, endIndex):
        return "".join(self._inputText[beginIndex: endIndex])
...
if parser.parse():
    ...
    fmtString = parser.getStartExpression() + formatter.formatSql(parser.getElemList()) + ";\n" + parser.getStartSpacing() + "/* @formatter:on */\n\n"
    clipboard.fill_clipboard(fmtString)
else:
    p = subprocess.Popen(["kdialog", "--title", "Error", "--msgbox", parser.getErrorMsg()] , stdout=subprocess.PIPE)
    retCode = p.wait()
